...
public class SegmentationProcess {
	...
	public static int lEnd[]=new int [20];
	 ...
	 static Bitmap Segmentedword;
	
	public  Bitmap[][][] segmentationMain(Bitmap bmp)
	{
		bmp              = imgref.NoiseRemover(bmp);
		bmp              = rescale(bmp);
		Bitmap array[][] = new Bitmap[50][20];int inc=0;
		bmp              = bmp.copy(Config.ARGB_8888, true);
		//int roi[]        = imgref.NoiseRemoverforROI(bmp);
		int roi[]        = imgref.getROIpoints(bmp);
		...
		for(int i=0;i<inc;i++)
		{
			...
			for(int z=1;z<wordSplit[i][0];z=z+2)
			{
				...
				//if(ModeSelectionActivity.Join == 0)
				{
					word = temp;
					result[i][inc1]=imgref.getCharacters(temp);
					Segmentedword = result[i][inc1][0];
				}
				/*else if(ModeSelectionActivity.Join == 1)
				{
					result[i][inc1]=working(temp);
					Segmentedword = result[i][inc1][0];
				}*/
	...
	public Bitmap[] working(Bitmap bmp)
	{
		Bitmap bmpseg[] = imgref.handwrittenSegmentation(bmp);
		return bmpseg;
	}
	...
	public static Bitmap rescale(Bitmap bmp)
	{
