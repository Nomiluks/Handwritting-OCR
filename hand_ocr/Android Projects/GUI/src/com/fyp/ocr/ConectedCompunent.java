...
public class ConectedCompunent {
	...
	public Bitmap invalidObjectRemovel(Bitmap bmp,Bitmap []objects,int inc)
	{
    	...
    	{
			int picw1 = objects [sa].getWidth() ;
			int pich1 = objects [sa] .getHeight() ;
			int arr[] = new int [picw1 * pich1 ];
			objects[sa].getPixels(arr, 0, picw1, 0, 0, picw1 , pich1 ); 
    		...
    		else
    		{
    			...
    			boolean res = false;
    			//res = checkHight1(bmp , s[0], (( s [0] + s [1])/2) , s[2] , s[3] );
    			...
    			else
    			{
    				...
    				//res =countPixle(arr, 0, picw1, pich1, s[0], (s[1]+s[0])/2, s[2], s[3]);
    	    		//android.util.Log.i(" sha " ,sa + " "+" "+ res+" e" );

    	    			//countPixle(pix, 5, picw, pich, s[0], w/2 , s[2] , s[3] );//countPixle(pix, 3, picw, pich, s[0], s[1] , s[2] , s[3]);

    				 /*if( countPixle(arr, 0, picw1, pich1, s[0], (s[1]+s[0])/2, s[2], s[3]) == true )
    					{
	...
	public int  outOfBoundHandle(int no , int size){
		if(no<0){
			no=0;
		}
		else
	...
	public boolean countPixle(int arr[] , int num , int picw ,int pich , int x1 , int x2 , int y1 , int y2 ){
		int index = 0 , count = 0 , limit = ( ( pich * picw ) / 100)*80  ;
		boolean noise = false;
		for ( int x = x1 ; x < x2 ; x++ ){
			for ( int y = y1 ; y < y2 ; y++ ){
				index = (y * picw ) + x ;
	...
	public int[]  resetlbl(int arr[] )  // reset label in sequnce //
    {
		...
		for (int y = 0; y < arr.length; y++){
			if(arr[y]!=0&&count<arr[y])	{
	    		...
	    		/*int index =0;
	    		for(int s=0;s<bmp.getWidth();s++)
				{
					//count=0;
		        	for(int z=0;z<bmp.getHeight();z++)	{
		        		index=z*bmp.getWidth()+s;
		        		if (arr[index] == count){
		        			if(count % 4 == 1)
		        				bmp.setPixel(s, z, Color.RED);
		        			if(count % 4 == 2)
		        				bmp.setPixel(s, z, Color.GREEN);
		        			if(count % 4 == 3)
		        				bmp.setPixel(s, z, Color.BLUE);
		        			if(count % 4 == 0)
		        				bmp.setPixel(s, z, Color.MAGENTA);
