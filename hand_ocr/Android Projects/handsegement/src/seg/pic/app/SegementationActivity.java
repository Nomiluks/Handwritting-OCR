...
public class SegementationActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) 
    {
        ...
        Bitmap bmp = BitmapFactory.decodeResource(getResources(), R.drawable.y3);//Get image from drawable
        bmp = getAdaptiveLocalBinarization(17,9,bmp);//binirization of image
        int picw = bmp.getWidth(),pich = bmp.getHeight();//getting height and width of an image
    	Bitmap bmp1=Bitmap.createBitmap(bmp, 0,0,picw, pich);
    	int roi[] = regionOfIntrest(bmp1, picw, pich);//getting of interest from image into an array
    	...
    	//Bitmap cra [] = con.conectedCompunentMain1(bmp);
    	//bmp1.setPixel(10, 15, Color.RED);
    	
    	//bmp1= getAdaptiveLocalBinarization(3, 3, bmp1);
     	//img.setImageBitmap(bmp1);
    	
    	
	    img.setImageBitmap(bmp);
	    struct res  = handWrittenSegmentation(bmp, bmp1);
	    img1.setImageBitmap(res.bmp[res.counter-1]);
	   // img1.setImageBitmap(handWrittenSegmentation(bmp,  bmp1)[0]);
	   // text.append(""+handWrittenSegmentation(bmp, bmp1));
	    text.append("" + res.counter);
    }
    ...
    public struct  handWrittenSegmentation(Bitmap orignalImage , Bitmap baseLineImage)
    {
    	...
    	//Bitmap finalResult [] = new Bitmap [50];
    	int inc = 0;
    	handwrittenSegmentation seg = new handwrittenSegmentation();
    	conectedCompunent con = new conectedCompunent();
    	...
    	struct cha =con.conectedCompunentMain1(orignalImage);
    	//int z=1;
    	for (int z = 1 ; z< cha .counter ; z++ )
    	{
    		...
    		int cHight = E_C_Roi[3]-E_C_Roi[2];
    		int cWidth = E_C_Roi[1]-E_C_Roi[0];
    	...
    	//if( cHight > cWidth )
    	{
        	...
        	{ 
        		for(int i =1 ; i<result[0] ; i+=2){
            		...
            		//orignalImage = changeColor( orignalImage ,cha[z], result[i] , result[i+1] , E_C_Roi[2], E_C_Roi[3]);
              	}
        		Bitmap temp = getCropedRegion(orignalImage, E_C_Roi[0], E_C_Roi[1], roi[2], roi[3] );
        		temp = getAdaptiveLocalBinarization(17, 9 , temp );
        		struct temp1  = con.conectedCompunentMain2(temp);
        		for(int i= 1; i< temp1.counter ;i++){
        			//finalResult[inc] = temp1[i];
   	 	...
   	 	for(int i =1 ; i<cracterSplit[0] ; i+=2)
   	 	{
   	 		if( cracterSplit[i+1]- cracterSplit[i]<dense*3)
	   		 {
			    	 ...
			    	 if( ( cracterHight ) < baseLineHight ) {
			    		// orignalImage.setPixel(cracterSplit[i], 10, Color.GREEN);
			    		// pre1=false;
				    	 ...
				    	 if(arr[0]<3) {
				    		 ...
				    		 if(distBet2Caracter > (cracterHight)) {
				    			 int sPoint = findBestSplitPoint(orignalImage, ( cracterSplit[i+1] + ( dense * 2) ) , ( cracterSplit[i+2]-3), orignalImageRoi[2], orignalImageRoi[3], dense);
			    		...
			    		// orignalImage.setPixel(cracterSplit[i], 10, Color.RED);
			    		...
			    		// else
			    		 if(( cracterSplit[i+2]==0 && (orignalImageRoi[1] - cracterSplit[i+1]) > (dense*2)+2)){
			    			 int sPoint = findBestSplitPoint(orignalImage, ( cracterSplit[i+1] + ( dense * 2) ), (orignalImageRoi[1]-3), orignalImageRoi[2] , orignalImageRoi[3], dense);
			    			 cracterSplit[i+1]=sPoint;
			    			 pre1=false;
			    		 }
    ...
    public int findBestSplitPoint( Bitmap img,int x1 , int x2 , int y1 , int y2 , int dense )
    {
    	int picw = img.getWidth();	int pich = img.getHeight();
    	int[] pix = new int[picw * pich];
	    img.getPixels(pix, 0, picw, 0, 0, picw, pich); 
	...
	public void findBestSplit(Bitmap img,int pix[],int split[],int picw, int pich, int y1 ,int dence)
	{
		int index=0;int count=0;
		 boolean butt=true,u=true,first=true,last=true;int inc=0;
		 int R=0,G=0,B=0;
		 //int []splip=new int[500];
		// int picw=img.getWidth(),pich=img.getHeight();
		 int counter=0,a=0;
		// int[] pix = new int[picw * pich];
		   
		    //img.getPixels(pix, 0, picw, 0, 0, picw, pich);
		    for (int y =1 ; y < split[0]; y++)
	        {
		    	count=0;inc=0;
		    	butt=true;u=true;
	        	for (int x = y1; x < pich; x++)
	            {
	        		 index = split[y] * picw+y;
	        	     R = (pix[index] >> 16) & 0xff; //bitwise shifting
	        		 G = (pix[index] >> 8) & 0xff;
	        		 B =  pix[index] & 0xff;
